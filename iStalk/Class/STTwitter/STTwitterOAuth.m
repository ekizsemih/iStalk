...
- (void)signRequestTokens:(STHTTPRequest *)r isMediaUpload:(BOOL)isMediaUpload oauthCallback:(NSString *)oauthCallback withAccessToken:(NSString *)accesstoken withAccessTokenSecret:(NSString *)accesstokensecret {
    NSParameterAssert(_oauthConsumerKey);
    NSParameterAssert(_oauthConsumerSecret);
    NSMutableArray *oauthParameters = [NSMutableArray arrayWithObjects:
                                       @{@"oauth_consumer_key"     : [self oauthConsumerKey]},
    ...
    if([oauthCallback length]) [oauthParameters addObject:@{@"oauth_callback" : oauthCallback}];
        ...
        [oauthParameters addObject:@{@"oauth_token" : accesstoken}];
...
- (void)signRequest:(STHTTPRequest *)r isMediaUpload:(BOOL)isMediaUpload withAccessToken:(NSString *)accesstoken withAccessTokenSecret:(NSString *)accesstokensecret{
//    [self signRequestTokens:r isMediaUpload:isMediaUpload oauthCallback:nil withAccessToken:accesstoken withAccessTokenSecret:accesstokensecret];
    
    [self signRequest:r isMediaUpload:isMediaUpload oauthCallback:nil];
}

- (void)signRequest:(STHTTPRequest *)r withAccessToken:(NSString *)accesstoken withAccessTokenSecret:(NSString *)accesstokensecret {
    //TWEET CEKERKEN
    [self signRequest:r isMediaUpload:NO withAccessToken:accesstoken withAccessTokenSecret:accesstokensecret];
}
...
- (NSDictionary *)OAuthEchoHeadersToVerifyCredentials {
    ...
    [self signRequest:r withAccessToken:nil withAccessTokenSecret:nil];
...
- (STHTTPRequest *)fetchResource:(NSString *)resource
    ...
    if([HTTPMethod isEqualToString:@"GET"]) {
        ...
        [self signRequest:r withAccessToken:accesstoken withAccessTokenSecret:accesstokensecret];
    }
    else {
